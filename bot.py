...
def tasks_inline_keyboard(company):
...
async def start(message: types.Message):
    ...
    await message.answer(workspace_text(user), reply_markup=private_keyboard())
        ...
        for ws in user["workspaces"].values():
            if ws["chat_id"] == chat_id and ws["thread_id"] == thread_id:
                ...
                msg = await message.answer(
                    ...
                    reply_markup=tasks_inline_keyboard(company)
                )
...
async def toggle_task_callback(callback: types.CallbackQuery):
            ...
            if ws["chat_id"] == chat_id and ws["thread_id"] == thread_id:
                for cname, company in ws["companies"].items():
                    if company["message_id"] == callback.message.message_id:
                        ...
                        await callback.answer()  # уведомление без сообщения в чат
                        return

@dp.callback_query_handler(lambda c: c.data == "add_task")
async def add_task_callback(callback: types.CallbackQuery):
    await callback.answer("Напиши новую задачу в личку боту", show_alert=True)
