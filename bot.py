...
def make_task(text: str, done: bool = False):
...
def normalize_data(data: dict) -> dict:
    ...
    for wid, ws in list(data["workspaces"].items()):
        ...
        for t_idx, task in enumerate(ws["template"]):
            if not isinstance(task, dict):
                ws["template"][t_idx] = make_task(str(task), False)
                task = ws["template"][t_idx]
        ...
        for idx, company in enumerate(ws["companies"]):
            ...
            for t_idx, task in enumerate(company["tasks"]):
                if not isinstance(task, dict):
                    ws["companies"][idx]["tasks"][t_idx] = make_task(str(task), False)
...
def format_days_left(task: dict) -> str:
    due_at = parse_iso_dt(task.get("deadline_due_at"))
...
def active_deadline_due_at(task: dict) -> datetime | None:
    if task.get("done"):
        return None
    return parse_iso_dt(task.get("deadline_due_at"))


def task_sort_key(task: dict):
    due_at = active_deadline_due_at(task)
    if due_at is None:
        return (1, datetime.max, task.get("created_at") or "")
    return (0, due_at, task.get("created_at") or "")
...
def sorted_template_task_entries(tasks: list[dict]):
    return sorted(
        list(enumerate(tasks)),
        key=lambda item: (
            1 if not isinstance(item[1].get("template_deadline_days"), int) or item[1].get("template_deadline_days") <= 0 else 0,
            item[1].get("template_deadline_days") if isinstance(item[1].get("template_deadline_days"), int) and item[1].get("template_deadline_days") > 0 else 10**9,
            item[1].get("created_at") or "",
        ),
    )


def task_icon(task: dict) -> str:
    if task.get("done"):
        return "✔"
    due_at = parse_iso_dt(task.get("deadline_due_at"))
    started_at = parse_iso_dt(task.get("deadline_started_at"))
    if not due_at or not started_at:
        return "⬜"
    now = now_utc()
    total = max(1.0, (due_at - started_at).total_seconds())
...
def task_button_text(task: dict, include_days: bool = False) -> str:
    suffix = format_days_left(task) if include_days else ""
    return f"{task_icon(task)} {task.get('text', '')}{suffix}"
...
def template_task_button_text(task: dict) -> str:
    ...
    return f"{task.get('text', '')}{suffix}"


def parse_deadline_input(text: str) -> tuple[str, int | datetime | None]:
    value = (text or "").strip()
    if not value:
        return ("invalid", None)
    if value.isdigit():
...
def apply_task_deadline(task: dict, raw_value: str) -> bool:
    kind, parsed = parse_deadline_input(raw_value)
    if kind == "invalid":
        return False
    if not task.get("deadline_started_at"):
        task["deadline_started_at"] = now_utc().isoformat()
    if kind == "days":
        task["deadline_due_at"] = (now_utc() + timedelta(days=int(parsed))).isoformat()
    else:
        task["deadline_due_at"] = parsed.isoformat()
    return True


def clear_task_deadline(task: dict):
    task["deadline_started_at"] = None
    task["deadline_due_at"] = None


def set_template_deadline_days(task: dict, raw_value: str) -> bool:
    ...
    if days <= 0:
        return False
    task["template_deadline_days"] = days
    return True


def clone_template_task_to_company(task: dict) -> dict:
    new_task = make_task(task.get("text", ""), False)
    deadline_days = task.get("template_deadline_days")
    if isinstance(deadline_days, int) and deadline_days > 0:
        start = now_utc()
        new_task["deadline_started_at"] = start.isoformat()
        new_task["deadline_due_at"] = (start + timedelta(days=deadline_days)).isoformat()
    return new_task
...
def company_card_text(company: dict) -> str:
    ...
    if company["tasks"]:
        for _, task in sorted_task_entries(company["tasks"]):
            lines.append(task_button_text(task, include_days=True))
    else:
        lines.append("—")
    return "\n".join(lines)
    ...
    for task_idx, task in sorted_task_entries(company["tasks"]):
        kb.add(
            InlineKeyboardButton(
                task_button_text(task, include_days=False),
...
def template_menu_kb(wid: str, ws: dict):
    kb = InlineKeyboardMarkup(row_width=1)
    for idx, task in sorted_template_task_entries(ws["template"]):
...
async def edit_template_item_menu(data: dict, wid: str, template_idx: int):
    ...
    await upsert_ws_menu(
        ws,
        f"⚙️ {template_task_button_text(task)}",
        template_item_kb(wid, template_idx, task),
    )
...
async def set_task_deadline_prompt(cb: types.CallbackQuery):
    await cb.answer()
    _, wid, company_idx, task_idx = cb.data.split(":")
    company_idx = int(company_idx)
    task_idx = int(task_idx)
    ...
    async with lock:
        ...
        await set_prompt(
            ws,
            "⏰ Введите дедлайн: дату в формате ДД.ММ.ГГГГ или число дней, например 3:",
            {
...
async def delete_task_deadline(cb: types.CallbackQuery):
    ...
    async with lock:
        ...
        if task_idx < 0 or task_idx >= len(company["tasks"]):
            return
        clear_task_deadline(company["tasks"][task_idx])
        await sync_company_everywhere(ws, company_idx)
        await edit_company_menu(data, wid, company_idx)
        await save_data_unlocked(data)
...
async def set_template_deadline_prompt(cb: types.CallbackQuery):
    await cb.answer()
    _, wid, template_idx = cb.data.split(":")
    template_idx = int(template_idx)
    ...
    async with lock:
        ...
        await set_prompt(
            ws,
            "⏰ Введите количество дней для шаблонной задачи, например 3:",
            {
                "type": "set_template_deadline",
                "template_idx": template_idx,
                "back_to": {"view": "template_task", "template_idx": template_idx},
            },
        )
        await save_data_unlocked(data)
    ...
    async with lock:
        ...
        if mode == "new_company":
            ...
            company = {
                "id": uuid.uuid4().hex,
                "name": text,
                "tasks": [clone_template_task_to_company(t) for t in ws["template"]],
                "card_msg_id": None,
                "mirror": None,
            }
        ...
        if mode == "new_task":
            ...
            ws["companies"][company_idx]["tasks"].append(make_task(text, False))
            ws["awaiting"] = None
        ...
        if mode == "set_task_deadline":
            ...
            if task_idx < 0 or task_idx >= len(company["tasks"]):
                ...
                await save_data_unlocked(data)
                return
            if not apply_task_deadline(company["tasks"][task_idx], text):
                await send_temp_message(ws["chat_id"], "Неверный формат. Введите дату ДД.ММ.ГГГГ или число дней.", ws["thread_id"], delay=6)
                await save_data_unlocked(data)
                return
            ws["awaiting"] = None
            await safe_delete_message(ws["chat_id"], prompt_msg_id)
        ...
        if mode == "set_template_deadline":
            template_idx = awaiting["template_idx"]
            if template_idx < 0 or template_idx >= len(ws["template"]):
                ws["awaiting"] = None
                await save_data_unlocked(data)
                return
            if not set_template_deadline_days(ws["template"][template_idx], text):
                await send_temp_message(ws["chat_id"], "Введите положительное число дней, например 3.", ws["thread_id"], delay=6)
                await save_data_unlocked(data)
                return
            ws["awaiting"] = None
            await safe_delete_message(ws["chat_id"], prompt_msg_id)
        ...
        if mode == "new_template_task":
            ws["template"].append(make_task(text, False))
            ws["awaiting"] = None
